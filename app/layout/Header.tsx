...
export function Header({ sections, active }: HeaderProps) {
  ...
  return (
    <header
      id="site-header"
      className="sticky top-0 z-40 backdrop-blur bg-white/75 border-b"
    >
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between">
        <button
          onClick={() => {
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm">
          {navSections.map((id) => (
            ...
            >
              {id.charAt(0).toUpperCase() + id.slice(1)}
              {active === id && (
                <span className="absolute -bottom-2 left-0 h-0.5 w-full bg-green-600 rounded-full" />
              )}
            </button>
          ))}
          <button
            onClick={() => scrollToId("contact")}
            className="px-4 py-2 rounded-full bg-green-700 text-white hover:bg-green-800 shadow-sm"
          >
            Book Free Call
          </button>
        </nav>
        ...
        <div className="md:hidden border-t bg-white/95 backdrop-blur">
          <div className="max-w-6xl mx-auto px-4 py-4 grid gap-2">
            ...
            <button
              ...
              className="mt-2 px-4 py-2 rounded-full bg-green-700 text-white hover:bg-green-800 shadow-sm"
            >
              Book Free Call
            </button>
          </div>
